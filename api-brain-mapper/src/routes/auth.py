...
def addUser():
    ...
    roleId = InputValidator.validate_integer(req["roleId"], "Role ID", min_value=1)
...
def register():
    ...
    name = InputValidator.validate_name(
        req["name"], field_name="Nombre", min_length=2, max_length=100
    )
    lastName = InputValidator.validate_name(
        req["lastName"], field_name="Apellido", min_length=2, max_length=100
    )
    email = InputValidator.validate_email_format(req["email"])
...
def login():
    # Validate request structure
    req = InputValidator.validate_json_request(request, ["email", "passwd"])
    ...
    if (
        user is None
    ):  # TODO: Should only lock if the email matches a user but the password is wrong
    ...
    responseData = {"loggedIn": True, "user": user_serialized}
...
def isLoggedIn():
    ...
    if not token:
        return jsonify({"loggedIn": False, "message": "No authentication token"}), 200
    ...
    responseData = {"loggedIn": isLoggedIn, "message": message}
...
@auth.route("/logout-all-sessions", methods=["POST"])
@auth_required()
def logout_all_sessions():
    ...
    """
    try:
        # Close all sessions for the current user
        UserSession.close_all_sessions(g.uid)
        db.session.commit()

        logger.info(f"All sessions closed for user {g.uid}")

        return (
            jsonify(
                {
                    "success": True,
                    "message": "All sessions have been closed successfully",
                }
            ),
            200,
        )
        ...
        logger.exception(f"Error closing all sessions for user {g.uid}: {str(e)}")
        db.session.rollback()
        return jsonify({"success": False, "message": "Error closing sessions"}), 500
