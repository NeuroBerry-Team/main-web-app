...
def get_user_stats():
    ...
    try:
        ...
        total_analyses = (db.session.query(Inference)
                          .filter_by(userId=user_id).count())
        
        analyses_this_week = (db.session.query(Inference)
                              .filter(Inference.userId == user_id,
                                      Inference.createdOn >= week_ago)
                              .count())
        
        # Session statistics
        last_login = (db.session.query(func.max(UserSession.loginAt))
                      .filter_by(userId=user_id).scalar())
        
        active_days_count = (db.session
                             .query(func.count(distinct(
                                 func.date(UserSession.loginAt))))
                             .filter(UserSession.userId == user_id,
                                     UserSession.loginAt >= month_ago)
                             .scalar() or 0)
        
        # Recent analyses (last 10)
        recent_analyses = (db.session.query(Inference)
                           .filter_by(userId=user_id)
                           .order_by(Inference.createdOn.desc())
                           .limit(10).all())
        
        # Recent sessions (last 10)
        recent_sessions = (db.session.query(UserSession)
                           .filter_by(userId=user_id)
            ...
            "recentSessions": [
                {
                    ...
                    "logoutAt": (session.logoutAt.isoformat() + 'Z'
                                if session.logoutAt else None),
...
def calculate_login_streak(user_id):
    ...
    try:
        ...
        login_dates = (db.session.query(
                        func.date(UserSession.loginAt).label('login_date'))
                      .filter_by(userId=user_id)
                      .distinct()
...
def get_user_inferences():
    ...
    try:
        ...
        if inference_id:
            inference = (db.session.query(Inference)
        ...
        # Get total count
        total_count = (db.session.query(Inference)
                      .filter_by(userId=user_id).count())
        
        # Get paginated inferences
        inferences = (db.session.query(Inference)
                     .filter_by(userId=user_id)
                     .order_by(Inference.createdOn.desc())
                     .limit(limit)
                     .offset(offset)
        ...
        return jsonify({
            "success": False,
            "error": "Failed to retrieve inferences"
        }), 500